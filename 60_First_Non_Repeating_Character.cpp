/*
   TCS NQT PYQ 60: First Non-Repeating Character
   Source: Verified PYQ
   Exam/Shift: 6 May 2024 Shift 2
   Difficulty: Easy
   Topics: String
...
int main(){string s; getline(cin,s); vector<int> f(256); for(unsigned char c:s) f[c]++; for(char c:s) if(f[(unsigned char)c]==1){cout<<c<<"\n"; return 0;} cout<<-1<<"\n";}
