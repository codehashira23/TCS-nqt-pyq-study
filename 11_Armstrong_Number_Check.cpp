/*
   TCS NQT PYQ 11: Armstrong Number Check
   Source: Verified PYQ
   Difficulty: Easy
   Topics: Math

   QUESTION
   A number is an Armstrong number if the sum of its own digits each raised to the power of the number of digits equals the original number. Determine if a given number is an Armstrong number.
...
int main(){long long n; cin>>n; string s=to_string(n); long long sum=0;
    for(char ch:s){ long long d=ch-'0', p=1; for(size_t i=0;i<s.size();i++) p*=d; sum+=p; }
